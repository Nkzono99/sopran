...
to raw sidecar records. Source files under `Store.root` are stored as
root-relative POSIX paths such as `raw/kaguya/...`; external input files may
remain absolute paths. `Store.verify_dataset(...)` checks both dataset shard
