...
この文書は、ユーザーに見える API と中核データモデルの source of truth とする。
保存、batch、surface、plotting の詳細は別文書に切り出し、ここでは public API との境界だけを
固定する。

| 文書 | 役割 |
| --- | --- |
| [spec.md](spec.md) | user-facing API, core data model, v0.1 contract |
| [store.md](store.md) | raw / normalized / features / models / databases, manifest, catalog |
| [pipeline.md](pipeline.md) | batch processing, lazy execution, Rust backend stages |
| [surface.md](surface.md) | Moon, DEM, SVM, SZA, shadow, illumination, map projection |
| [plotting.md](plotting.md) | PlotStack, quicklook, plotting backend policy |
...
実 dtype も照合する。`VariableSchema.units` / `VariableSchema.frame` が設定されていて、
xarray variable または DataArray attrs に `units` / `frame` が存在する場合は、それらの
metadata も照合する。欠損、dims mismatch、dtype mismatch、units mismatch、frame mismatch は
public exception の `SchemaError` として扱う。
units / frame metadata がまだ付いていない既存データは、移行期間として validation error にしない。
...
この節は public API の境界を示す。DEM/SVM/SZA/shadow/illumination の詳細仕様は
[surface spec](surface.md) に切り出す。v0.1 では `spn.Moon()` の API skeleton と source discovery 程度に留め、
...
shape、datum、座標 convention、projection、area_or_point を継承する。
明示指定された convention は canonical value に正規化して保存する。
...
公開ドキュメントは日英両対応を前提にする。利用者がページ上で
`Lang: 日本語/English` を切り替えられる導線を必ず持たせる。
この切り替えは公開 docs、package 内 guide、notebook 表示のいずれでも同じ概念として扱い、
ユーザーに見える label は原則として `Lang: 日本語/English` に統一する。
各ページの先頭付近に現在言語と切り替え先が分かる language switcher を置き、API から
取得する `GuidePage` でも同じ表記を Markdown 表示に含める。

必須要件:

- 公開 docs、package guide、notebook 表示の language switcher 表記は
  `Lang: 日本語/English` を canonical label とする。
- 公開 docs の header / navigation には `Lang: 日本語/English` の切り替え UI を置き、
  日本語版と English 版を同じ階層の対応ページとして行き来できるようにする。
- language switcher は現在言語、切り替え先、fallback 先を利用者に分かる形で示す。
- API から取得する `GuidePage` も docs と同じ言語状態を持ち、`language="ja"` /
  `language="en"` で明示的に切り替えられる。

方針:

- 日本語を primary language とし、英語を parallel language として整備する。
- `guide()` / `help()` の既定言語は日本語 (`language="ja"`) とし、英語は
  `language="en"` で明示的に選ぶ。
- URL は言語ごとに安定させる。例: `/ja/...` と `/en/...`。
- 各ページには対応する翻訳ページへの link または language switcher を置く。
- language switcher の表示名は `Lang: 日本語/English` を canonical label とし、
  navigation、notebook 表示、package guide で別表記を増やさない。UI ではこの label で
  日本語版と English 版を切り替える。
...
kg.guide()            # KAGUYA README 日本語版を参照
...
page.urls        # 言語別 docs URL。未設定なら url に fallback
...
- `TimeBins.to_polars()` は `index`, `start`, `stop`, `center`, `duration_seconds`,
  `is_partial` の bin table を返し、`TimeBins.metadata()` は edges、centers、duration、
  partial flag を含む provenance 用 dict を返す。
...
backend は同じ run ID と download policy を
`dataset.json["provenance"]["pipeline"]` に残し、quicklook を生成する場合は quicklook
metadata にも同じ run ID と download policy を入れる。KAGUYA ESA1 の
dataset-writing pipeline は `dataset_root/logs/<run_id>.json` に structured log を保存し、
run mode、download policy、started_at、finished_at、stage list、output target、shard row count、
...
metadata output なら artifact path / format、metadata path、metadata を含める。summary 内の
path は OS に依存しない POSIX-style string (`/` 区切り) とする。
...
- DEM/SVM/SZA/shadow/illumination は mission-first ではなく `spn.Moon()` など body-first API を主導線にする。
...
- data store は raw / normalized / features / models / databases の 5 層。
