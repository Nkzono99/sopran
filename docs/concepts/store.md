...
When a pipeline writes to `db.product(..., description=...)`, the completed
dataset is registered in `database.json` after `run()` succeeds and the
description is kept as database product metadata.
