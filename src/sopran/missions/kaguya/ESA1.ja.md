...
- `energy_flux`: PACE INFO table の g-factor を使って `counts` から較正した energy flux。
- `energy`: 未較正時は channel index。較正済みロードでは PACE INFO table 由来の
  eV 単位の energy center になり、元の channel は `energy_channel` に残ります。
...
SOPRAN は `counts / (integ_t * gfactor * efficiency)` で `energy_flux` を計算します。
既定の `efficiency` は 0.6 です。endpoint からも table は自動で読みます。
