...
def test_store_supports_models_layer_datasets(tmp_path) -> None:
    ...
    dataset = store.write_parquet_dataset(
        dataset_id="analysis.wake_classifier.score",
        layer="models",
        mission="analysis",
        instrument="wake_classifier",
        product="score",
        schema=schema,
        time_coverage=time,
        frame=pl.DataFrame({"time": [time.start_iso], "score": [0.5]}),
    )

    assert store.models_path("analysis", "wake_classifier", "score") == (
        tmp_path / "store" / "models" / "analysis" / "wake_classifier" / "score"
    )
    assert dataset.root == store.dataset_path(
        "analysis.wake_classifier.score",
        layer="models",
    )
    assert dataset.manifest()["layer"] == "models"
    assert store.dataset("analysis.wake_classifier.score").root == dataset.root
    assert store.datasets(layer="models", refresh=True).select(
        "dataset_id"
    ).to_series().to_list() == ["analysis.wake_classifier.score"]


def test_store_validates_product_schema_before_writing_parquet(tmp_path) -> None:
    store = Store(tmp_path / "store")
    time = spn.period("2008-02-01", "2008-02-02")

    with pytest.raises(spn.SchemaError, match="missing variables: counts"):
        store.write_parquet_dataset(
            ...
            schema=KAGUYA_ESA1_SCHEMA,
            time_coverage=time,
            frame=pl.DataFrame({"time": ["2008-02-01T00:00:08Z"], "cnt": [64]}),
        )
...
def test_dataset_record_verifies_catalog_shard_checksums_by_status(tmp_path) -> None:
    store = Store(tmp_path / "store")
    first = spn.day("2008-02-01")
    second = spn.day("2008-02-02")
    kwargs = {
    ...
    assert not dataset.verify_checksums()
    assert dataset.verify_checksums(status="complete")
    assert not dataset.verify_checksums(status="failed")
    with pytest.raises(ValueError, match="shard status"):
        dataset.verify_checksums(status="unknown")
...
def test_dataset_record_lists_shards_by_status(tmp_path) -> None:
    store = Store(tmp_path / "store")
    first = spn.day("2008-02-01")
    second = spn.day("2008-02-02")
    kwargs = {
    ...
    assert [shard["path"] for shard in dataset.shards()] == [
        "shards/part-000.parquet",
        "shards/part-001.parquet",
    ]
    assert dataset.shards(status="failed") == dataset.failed_shards()
    assert dataset.failed_shards() == (
        {
            "path": "shards/part-001.parquet",
            "schema_version": "0.1",
            "start": second.start_iso,
            "stop": second.stop_iso,
            "row_count": 1,
            "checksum": dataset.shards(status="failed")[0]["checksum"],
            "status": "failed",
        },
    )
    with pytest.raises(ValueError, match="shard status"):
